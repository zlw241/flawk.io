  ...
  _createClass(Player, [{
    ...
    value: function update(velocityScale, worldWidth, worldHeight) {
      ...
      for (var _i = 0; _i < this.zombies.length; _i++) {
        ...
        nextVel = this.zombies[_i][2];
        this.zombies[_i] = [zX, zY, zV];
      }
...
var Game = function () {
  function Game(context, canvas, socket) {
    ...
    this.viewport = new _viewport2.default(0, 0, 800, 800, this.world.width, this.world.height);
    this.nectar = [];
    this.players = {};
  ...
  _createClass(Game, [{
    ...
    value: function handleInput() {
      ...
      document.addEventListener("keydown", function (e) {
        if (e.key === " ") {
          if (_this3.players[window.playerId]) {
            _this3.players[window.playerId].boost();
          }
        }
      });
      ...
      document.addEventListener("keyup", function (e) {
        if (e.key === " ") {
          if (_this3.players[window.playerId]) {
            _this3.players[window.playerId].unboost();
          }
        }
      });
      ...
      document.addEventListener('mousemove', function (e) {
        ...
        var startRadians = Util.vectorToRadians(_this3.players[_this3.currentPlayerId].velX, _this3.players[_this3.currentPlayerId].velY);
        var targetRadians = Util.vectorToRadians(dX, dY);

        var _calculateNewVector = _this3.calculateNewVector(startRadians, targetRadians),
            _calculateNewVector2 = _slicedToArray(_calculateNewVector, 2),
            newX = _calculateNewVector2[0],
            newY = _calculateNewVector2[1];

        _this3.socket.emit("player move", {
          id: _this3.currentPlayerId,
          pos: [newX, newY]
...
var GameView = function () {
  ...
  _createClass(GameView, [{
    ...
    value: function update(data) {
      ...
      Object.values(data.players).forEach(function (d) {
        var player = _this.game.players[d.id];
        if (player) {
          player.update(0, _this.game.worldWidth, _this.game.worldHeight);
        } else {
          var existingPlayer = new _player2.default(d.id, d.pos[0], d.pos[1], d.zombies.length, _this.game, d.color);
          existingPlayer.radius = d.radius;
          _this.game.players[d.id] = existingPlayer;
        }
      }, this);
    ...
    key: 'updatePlayerPos',
    value: function updatePlayerPos(id, pos, speed, vel, radius, size) {
      var player = this.game.players[id];
      if (player) {
        // debugger
        this.game.players[id].x = pos[0];
        this.game.players[id].y = pos[1];
        player.speed = speed;
    ...
    value: function draw(data) {
      ...
      Object.values(data.players).forEach(function (player) {
        // debugger
        _this2.updatePlayerPos(player.id, player.pos, player.speed, player.vel, player.radius, player.size);
      }, this);
      this.game.nectar = [];
      Object.values(data.nectar).forEach(function (nectar) {
        _this2.game.addNectar(nectar.x, nectar.y, nectar.radius, nectar.color);
      });
      // debugger
