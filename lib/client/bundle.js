...
var Game = function () {
  ...
  _createClass(Game, [{
    ...
    value: function update(delta) {
      ...
      var normal_frame_time = Game.PHYSICS_FRAME_RATE;
  ...
  _createClass(GameView, [{
    ...
    value: function draw(data) {
      ...
      if (this.game.players[this.currentPlayerId]) {
        var currentPlayer = this.game.players[this.currentPlayerId];
        var input = data.players[this.currentPlayerId].lastInput;
        var playerData = data.players[this.currentPlayerId];

        var unprocessedInputs = [];

        if (currentPlayer.inputs.length > 0) {
          currentPlayer.inputs.forEach(function (i) {
            if (i.seq === input) {
              console.log('1', playerData.pos[0] - i.x, playerData.pos[1] - i.y);
              console.log('2', playerData.pos[0] - currentPlayer.x, playerData.pos[1] - currentPlayer.y);
            } else if (i.seq > input) {
              unprocessedInputs.push(i);
            }
          });
        }
        currentPlayer.inputs = unprocessedInputs;
      }
      ...
      if (this.game.players[this.currentPlayerId]) {
        var _currentPlayer = this.game.players[this.currentPlayerId];
        //   const input = data.players[this.currentPlayerId].lastInput;
        //   const playerData = data.players[this.currentPlayerId];
        //
        //   const unprocessedInputs = []
        //
        //   if (currentPlayer.inputs.length > 0) {
        //     currentPlayer.inputs.forEach((i) => {
        //       if (i.seq === input) {
        //         console.log(playerData.pos[0] - i.x, playerData.pos[1] - i.y);
        //       } else if (i.seq > input) {
        //         unprocessedInputs.push(i);
        //       }
        //     });
        ...
        this.game.viewport.update(_currentPlayer.x, _currentPlayer.y);
