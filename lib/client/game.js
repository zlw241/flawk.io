...
class Game {
  ...
  addNectar(id, x, y, r, c) {
    ...
    // const id = ++this.nectarIds
    const nectar = new Nectar(id, xPos, yPos, radius, c);
    this.add(nectar);
  }
  ...
  updatePlayerVelocity(targetRadians) {
    ...
    if (player) {
      ...
      player.inputs.push({ seq: this.inputs, x: player.x, y: player.y, vel: [newX, newY] })

      if (player.inputs.length > 100) {
        console.log('hehreherherh')
